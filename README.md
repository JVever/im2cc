...
im2cc 不是「同步消息」或「复制对话」。它直接操控你电脑上的 Claude Code 进程——手机和电脑用的是**同一个 session 文件**。Claude 记得之前所有的对话、读过的文件、做过的改动。

### 在哪里都是同一套操作

不管是在电脑终端、飞书群、还是微信里，用的都是同一组命令：

- `fn` 创建对话 · `fc` 接入对话 · `fl` 列出对话
- `fk` 终止对话 · `fd` 断开连接 · `fs` 查看状态

学一次，到处用。

### 同时支持飞书和微信

你可以两个都连——飞书群用来团队协作，微信用来个人使用。im2cc 自动管理独占访问：当你在微信接入一个对话，飞书端会自动断开，反过来也一样。

### 分叉感知

用 SparkChat 等工具分叉出来的 Claude Code 对话？也能流转到手机上。一句 `fc my-fork 2961` 就能注册并接入任何未管理的对话。

### 电脑端零感知

回到电脑后，`fc myproject` 一敲，tmux 窗口打开，你就在 Claude Code 里了——所有手机上的操作结果都在。不需要手动同步，不需要复制粘贴。
