...
```text
请帮我安装并配置 im2cc，让我可以通过飞书或微信在手机上远程操控这台电脑上的 AI coding tool。这个仓库是公开仓库，请优先使用普通 git HTTPS clone，不要依赖 gh CLI 登录，也不要依赖需要 GitHub 认证的 API / MCP / 集成。im2cc 采用隐藏安装目录 `~/.im2cc-app/`（与运行时数据目录 `~/.im2cc/` 区分）。请先执行：

git clone https://github.com/JVever/im2cc.git ~/.im2cc-app
...
如果这两种方式都失败，再排查 github.com 网络连通性、代理、文件系统权限或公司网络策略，不要默认要求我登录 gh。然后进入 ~/.im2cc-app，阅读 INSTALL.md、README.md 和仓库内的 agent/onboarding instructions，继续完成全部安装、IM 接入、首次验证、开机自启动和基础安全加固配置。只有在必须由我操作时再问我。
```
...
- 如果更新前 daemon 正在运行，自动重启让新版本生效

你的运行时配置保存在 `~/.im2cc/`，正常更新不会覆盖这部分内容。旧命令 `im2cc upgrade` 仍兼容，但不再作为主命令推荐。
