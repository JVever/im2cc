...
你的电脑上可以同时开着多个 Claude Code 对话，每个负责不同的项目。im2cc 让你在手机上也能同时管理它们：飞书中开几个群，每个群用 `/fc` 接入不同的项目——后端群让 Claude 改 API，前端群让它调样式，运维群让它查日志——多条线并行推进，互不干扰。

回到电脑后，每个项目的 tmux 窗口里都是最新的状态，直接继续。

### 飞书 + 微信双通道

国内两大主流 IM 都支持。飞书群天然适合多项目并行——一个群对应一个项目，群内所有人都能看到 Claude 的工作进展；微信适合个人使用——随时随地用最常用的 App 操控 Claude，不需要打开额外的工具。两个通道可以同时连接，im2cc 自动管理独占访问，不会出现冲突。
...
电脑终端里敲 `fc auth`，飞书群里发 `/fc auth`，微信里发 `/fc auth`——效果完全一样。没有额外的学习成本，也不需要记不同平台的不同操作方式。

### 回到电脑，零感知衔接

手机上让 Claude 改了代码、跑了测试、装了依赖——回到电脑后 `fc myproject` 一敲，tmux 窗口打开，所有改动都在。不需要手动同步，不需要复制粘贴，不需要重新建立上下文。就像你一直坐在电脑前一样。
